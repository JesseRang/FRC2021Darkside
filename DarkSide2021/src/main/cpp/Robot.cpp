...
//      Drive Train     //
DifferentialDrive ArcadeDrive(robot.LDriveMotor, robot.RDriveMotor);
...
void Robot::RobotInit()
{
  ...
  robot.intakeMotor.ConfigContinuousCurrentLimit(5, 0);
  ...
  robot.conveyorMotor.ConfigContinuousCurrentLimit(5, 0);
  robot.indexMotor.SetSmartCurrentLimit(5);
  ...
  robot.conveyorMotor.SetInverted(false); //was true, then motor got flipped physically
  robot.conveyorMotor.SetNeutralMode(Brake);
  robot.indexMotor.SetInverted(true);
  intakeFollower.Follow(robot.intakeMotor);
  ...
  */
  ...
  robot.LDriveMotor.RestoreFactoryDefaults();
  robot.RDriveMotor.RestoreFactoryDefaults();
  robot.LDriveMotor2.Follow(robot.LDriveMotor);
  robot.RDriveMotor2.Follow(robot.RDriveMotor);
...
void auton1()
{
  ...
  else if (phase1 < timer.Get() && timer.Get() < phase2)
  {
    ...
    robot.conveyorMotor.Set(0.5);
    robot.indexMotor.Set(0.7);
    ArcadeDrive.ArcadeDrive(0, 0);
  }
  else if (phase2 < timer.Get() && timer.Get() < phase3)
  {
    ...
    robot.flyWheelL.Set(0);
    robot.conveyorMotor.Set(0);
    robot.indexMotor.Set(0);
...
void auton2(double m_LimelightTurnCmd, bool m_LimelightHasTarget, double limelightSpeed)
{
  ...
  if (phase == 8 || phase == 9 || phase == 10 || phase == 11)
  {
    ...
    robot.vMotor1.Set(0.7);
    robot.vMotor2.Set(0.9);
    ...
    if (!breakBeamFull.Get() && !breakBeamNewBall.Get() && !breakBeamFourthBall.Get() && !breakBeamFifthBall.Get())
      ...
      robot.indexMotor.Set(0.5); //changed from .25

    if (!breakBeamFull.Get())
      robot.conveyorMotor.Set(0);
    else if (!breakBeamNewBall.Get())
      robot.conveyorMotor.Set(0.3);
    else
      robot.conveyorMotor.Set(0);
  }
}
...
void Robot::TeleopPeriodic()
{
  ...
  */
  ...
  if (rightTrigger0)
  {
    ...
    {
      robot.conveyorMotor.Set(1);
      robot.indexMotor.Set(.9);
    }
    else if (btnX1) // Medium
    {
      robot.conveyorMotor.Set(0.7);
      robot.indexMotor.Set(.9);
    }
    else if (btnA1) // Low
    {
  ...
  else if (leftBumper0) //Intake minus the intake motor
  {
    ...
    robot.vMotor1.Set(0.7);
    robot.vMotor2.Set(0.9);
    robot.indexMotor.Set(0.5);

    if (!breakBeamFull.Get() && !breakBeamNewBall.Get() && !breakBeamFourthBall.Get() && !breakBeamFifthBall.Get())
      robot.indexMotor.Set(0);
    else
      robot.indexMotor.Set(0.25);
  ...
  else if (leftTrigger1) // Purge the system
  {
    ...
    robot.conveyorMotor.Set(-0.5);
    robot.indexMotor.Set(-1);
  }
  else if (rightTrigger1) // reverse the "v"
  {
    robot.vMotor1.Set(-0.5);
    robot.vMotor2.Set(-0.5);
  }
  else
  {
    robot.intakeMotor.Set(0);
  ...
  robot.climbL.Set(rightAxisY1);
  robot.climbR.Set(rightAxisY1);

  if (leftBumper1)
    sbrakeLift.Set(false);
  else
    brakeLift.Set(true);
